...
// during `p-ui migrate-db --dsn` (SQLite → PostgreSQL data migration) and in
// related tests.
//
...
func MigrateData(srcPath, dstDSN string) error {
	...
	log.Println("Set PUI_DB_TYPE=postgres and PUI_DB_DSN=... in /etc/default/p-ui, then restart p-ui.")
	return nil
}
...
func PrepareSQLiteForMigration(dbPath string) error {
	...
	for _, table := range []string{"users", "settings", "inbounds"} {
		if !sqliteTableExists(sqlDB, table) {
			return fmt.Errorf("not a Penhoon UI panel database: required table %q is missing", table)
		}
	}
	for _, m := range migrationModels() {
