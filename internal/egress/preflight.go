...
*/
func (m *Manager) Preflight(ctx context.Context, gatewayBase netip.Prefix) Report {
	var report Report
	...
	if value, err := m.plane.Sysctl(ctx, IPForwardKey); err == nil && value == "0" {
		report.Notes = append(report.Notes, fmt.Sprintf(
			"%s is 0, so no L3 inbound on this host forwards a packet at all, egress or not", IPForwardKey))
	}
	return report
}
