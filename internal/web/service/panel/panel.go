...
const (
	panelUpdaterURL      = "https://raw.githubusercontent.com/Arman2122/p-ui/main/update.sh"
...
// GetUpdateInfo checks GitHub for the latest Penhoon UI release. When the dev channel
...
func (s *PanelService) startUpdate(useDev bool) (int64, error) {
	...
	runIDEnv := "PUI_UPDATE_RUN_ID=" + strconv.FormatInt(runID, 10)
	statusFileEnv := "PUI_UPDATE_STATUS_FILE=" + statusFile

	if systemdRun, err := exec.LookPath("systemd-run"); err == nil {
		unitName := fmt.Sprintf("p-ui-web-update-%d", time.Now().Unix())
		cmd := exec.CommandContext(context.Background(), systemdRun,
			"--unit", unitName,
			"--setenv", "PUI_MAIN_FOLDER="+mainFolder,
			"--setenv", "PUI_SERVICE="+serviceFolder,
			"--setenv", "PUI_UPDATE_TAG="+updateTag,
			"--setenv", runIDEnv,
	...
	cmd.Env = append(os.Environ(),
		"PUI_MAIN_FOLDER="+mainFolder,
		"PUI_SERVICE="+serviceFolder,
		"PUI_UPDATE_TAG="+updateTag,
		runIDEnv,
		statusFileEnv,
	)
...
func downloadPanelUpdater() (string, error) {
	...
	file, err := os.CreateTemp("", "p-ui-update-*.sh")
	if err != nil {
		return "", err
	}
	path := file.Name()
...
func fetchPanelRelease(tag string) (*service.Release, error) {
	url := "https://api.github.com/repos/Arman2122/p-ui/releases/latest"
	if tag != "" {
		url = "https://api.github.com/repos/Arman2122/p-ui/releases/tags/" + tag
	}
...
func resolveUpdateFolders() (string, string) {
	mainFolder := os.Getenv("PUI_MAIN_FOLDER")
	if mainFolder == "" {
	...
	if mainFolder == "" {
		mainFolder = "/usr/local/p-ui"
	}

	serviceFolder := os.Getenv("PUI_SERVICE")
	if serviceFolder == "" {
