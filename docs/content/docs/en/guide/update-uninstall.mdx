...
| `p-ui migrateDB [in] [out]` | Convert the SQLite `.db` ↔ a portable `.dump` (direction auto-detected) |
...
<Callout type="info">
  Migrating an existing SQLite install to PostgreSQL is a *panel* command rather
  than a menu subcommand — see [Database](/docs/reference/database).
</Callout>
...
<Callout type="warn">
  Uninstalling removes the database (`/etc/p-ui/p-ui.db`) and your configuration.
