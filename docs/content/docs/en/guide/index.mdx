...
**Penhoon UI** is a web control panel that sits on top of
...
| Database     | PostgreSQL                                   |
| Host         | Linux (Ubuntu 22.04+/Debian 12+) with systemd |
| Proxy engine | Xray-core (bundled and managed by the panel) |

The panel connects to PostgreSQL through the required
[`PUI_DB_DSN`](/docs/reference/env-vars#database) and listens on port **2053** by
default. Both are configurable — see [First login](/docs/guide/first-login) and
the environment variable reference.

## Who it's for

Penhoon UI is aimed at anyone running their own Xray server: from a single personal
...
<Callout type="info">
  Penhoon UI is a fork of [3x-ui](https://github.com/MHSanaei/3x-ui) by MHSanaei — itself an
  enhanced fork of the original x-ui panel. That lineage brings broader protocol support,
  improved stability, per-client traffic accounting, and multi-node management; Penhoon UI
  continues from there with its own changes.
</Callout>
