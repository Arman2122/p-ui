---
title: First Login
description: Find your generated Penhoon UI credentials, reach the panel, enable two-factor auth, and harden it before exposing anything.
icon: KeyRound
---
...
```bash title="/etc/p-ui/install-result.env (mode 600)"
PUI_USERNAME=...
PUI_PASSWORD=...
PUI_PANEL_PORT=...
PUI_WEB_BASE_PATH=...
...
```bash
p-ui              # menu → 11 (View Current Settings)
p-ui settings     # or the one-shot form
```

For **Docker**, read the generated credentials from the container logs, or run
`docker exec -it <container> p-ui setting -show`.
...
Penhoon UI supports TOTP two-factor auth (compatible with Google Authenticator, Aegis,
...
step or with `p-ui setting -resetTwoFactor`.
...
Use a valid certificate (via the `p-ui` menu's SSL management, or a reverse
