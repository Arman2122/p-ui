---
title: Installation
description: Install p-ui via the official script (stable, pinned, or dev-latest), unattended/cloud-init, or Docker — and choose SQLite or PostgreSQL.
icon: Download
---

p-ui runs on a wide range of Linux distributions — Ubuntu, Debian, Armbian,
...
and installs the `p-ui` management command.
...
To run with the bundled **PostgreSQL** service, uncomment the two `PUI_DB_*`
...
docker run -d \
  --cap-add=NET_ADMIN \
  --cap-add=NET_RAW \
  -e PUI_ENABLE_FAIL2BAN=true \
  -v $PWD/db/:/etc/p-ui/ \
  -v $PWD/cert/:/root/cert/ \
  --network=host \
  --restart=unless-stopped \
  --name p-ui \
  ghcr.io/arman2122/p-ui:latest
```

The `db/` volume holds the SQLite database (`/etc/p-ui/p-ui.db`) and `cert/`
...
[releases page](https://github.com/Arman2122/p-ui/releases), extract it, and run
...
- **SQLite** (default) — a single file at `/etc/p-ui/p-ui.db`. Zero setup.
...
```bash
PUI_NONINTERACTIVE=1 bash <(curl -Ls https://raw.githubusercontent.com/Arman2122/p-ui/master/install.sh)
```
