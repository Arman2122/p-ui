...
description: Fix common Penhoon UI problems — the panel won't start, 502 errors, certificate issues, and clients that can't connect.
icon: Wrench
---

A checklist for the most common issues. When in doubt, raise the log level
(`PUI_LOG_LEVEL=debug`) and check the panel and Xray logs.

## The panel won't start

- Check the service status and logs (`p-ui` menu → status/logs).
- Make sure the **panel port isn't already in use** by another service.
- Verify `PUI_DB_DSN` is set in `/etc/default/p-ui` and that PostgreSQL is
  running and reachable with those credentials — the panel exits at startup when
  it cannot connect.
...
<Callout type="info">
  ...
  [GitHub issues](https://github.com/Arman2122/p-ui/issues) — your symptom has
