...
description: Every Penhoon UI panel setting — web server, TLS, display, security, and notifications — with defaults from the source.
...
| ------------------- | ----------------------- | ----------------------------------------------------------------------- |
| `webPort`           | `2053`                  | Panel port (1–65535). The `PUI_PORT` env var overrides it at runtime.   |
...
launch with [`PUI_INIT_WEB_BASE_PATH`](/docs/reference/env-vars).

### TLS

Serving the panel over HTTPS protects your credentials in transit. Either set
`webCertFile` + `webKeyFile` — the [`p-ui` SSL menu](/docs/config/ssl-certificates)
