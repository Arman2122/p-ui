...
description: Get and renew TLS certificates for the Penhoon UI panel and inbounds — with the built-in ACME menu (domain or bare IP), a Cloudflare DNS-01 wildcard, or manual Certbot.
icon: ShieldCheck
---
...
- **The `p-ui` menu** — built-in [ACME](https://en.wikipedia.org/wiki/Automatic_Certificate_Management_Environment)
...
## The `p-ui` SSL menu (Let's Encrypt)

Run `p-ui` and choose **`20` — SSL Certificate Management**. It drives
...
`p-ui` → `20` → **Get SSL (Domain)**, then enter the domain. acme.sh requests
...
`p-ui` → **`21` — Cloudflare SSL Certificate**. When asked, choose **`t`** for an
