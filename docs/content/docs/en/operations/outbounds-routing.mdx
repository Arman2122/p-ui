...
description: Shape egress in p-ui — WARP and NordVPN outbounds, outbound subscriptions (server pools), routing rules, and load balancers.
icon: Route
---

Inbounds accept clients; **outbounds** decide where their traffic goes next.
p-ui can route traffic through Cloudflare WARP, NordVPN, or arbitrary outbound
...
shapes p-ui uses: **VLESS is the flat form** (`address`/`port`/`id`/`flow`/
...
WARP lets your server egress through Cloudflare's network. p-ui can register a
...
From the panel's WARP controls, register an account. p-ui fills the outbound's
...
p-ui can fetch NordVPN (NordLynx/WireGuard) credentials from an access token (or
