...
description: Shape egress in Penhoon UI — WARP and NordVPN outbounds, outbound subscriptions (server pools), routing rules, and load balancers.
icon: Route
---

Inbounds accept clients; **outbounds** decide where their traffic goes next.
Penhoon UI can route traffic through Cloudflare WARP, NordVPN, or arbitrary outbound
...
shapes Penhoon UI uses: **VLESS is the flat form** (`address`/`port`/`id`/`flow`/
...
WARP lets your server egress through Cloudflare's network. Penhoon UI can register a
...
From the panel's WARP controls, register an account. Penhoon UI fills the outbound's
...
Penhoon UI can fetch NordVPN (NordLynx/WireGuard) credentials from an access token (or
