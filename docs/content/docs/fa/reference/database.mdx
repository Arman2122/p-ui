---
title: پایگاه داده
description: بک‌اندهای ذخیره‌سازی p-ui — SQLite (پیش‌فرض) و PostgreSQL — مسیر پایگاه داده، استخر اتصال و مهاجرت از SQLite به PostgreSQL.
icon: Database
---

p-ui همه چیز را — inboundها، کلاینت‌ها و تنظیمات — در یک پایگاه داده ذخیره می‌کند.
...
سپس `PUI_DB_TYPE` و `PUI_DB_DSN` را در `/etc/default/p-ui` تنظیم کرده و سرویس را مجدداً
راه‌اندازی کنید:
