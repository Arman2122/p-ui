...
| `p-ui migrateDB [ورودی] [خروجی]` | تبدیل پایگاه‌دادهٔ SQLite بین `.db` و `.dump` (جهت به‌صورت خودکار تشخیص داده می‌شود؛ پیش‌فرض ورودی `/etc/p-ui/p-ui.db` است) |
...
<Callout type="info">
  مهاجرت SQLite ← PostgreSQL زیردستور مستقیمی در `p-ui` ندارد؛ آن را از گزینهٔ منوی
  **۲۵** (PostgreSQL Management) اجرا کنید — نگاه کنید به
...
<Callout type="info">
  ...
  به‌روزرسانی `p-ui`، ایمیج جدید را pull کرده و کانتینر را از نو بسازید
...
<Callout type="warn">
  حذف، پایگاه‌داده (`/etc/p-ui/p-ui.db`) و پیکربندی شما را پاک می‌کند. اگر ممکن است
